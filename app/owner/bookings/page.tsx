...
export default async function OwnerBookingsPage() {
  ...
  // Fetch bookings for owner's vessels
    ...
    <main className="mx-auto max-w-7xl p-6">
      ...
      {bookings.length === 0 ? (
        <Card className="p-12 text-center">
          ...
          <h3 className="mb-2 text-xl font-semibold">No booking requests yet</h3>
          <p className="mb-6 text-gray-600">
            Booking requests will appear here when operators request your vessels
          </p>
          <Link href="/owner/vessels">
            <Button>Manage Vessels</Button>
          </Link>
        </Card>
      ) : (
                  ...
                  <div className="flex-1 p-6">
                    ...
                    <div className="flex gap-3">
                      <Link href={`/owner/bookings/${booking.id}`}>
                        <Button>{isPending ? "Review Request" : "View Details"}</Button>
                      </Link>
                    </div>
                  </div>
                </div>
